...
def main():
  ...
  for i in range(n_iterations):
    boot_dataset = resample(dataset, replace=False, random_state=args.seed + i)

    current_params = {k: random.uniform(v[0], v[1]) for k, v in search_space.items()}
    ...
    try:
      ...
      train_loader, val_loader, test_loader = make_bucketed_loader(
        ...
        test_size=0.2, batch_size=args.batch_size, seed=args.seed + i)
      ...
      res_entry = current_params | perf_metrics
